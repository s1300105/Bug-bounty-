...
### 3. ワークフローを実行
このスキルディレクトリに同梱の `build-textbook.workflow.js` を、その**絶対パス**で実行する（`$HOME/.claude/skills/textbook-builder/build-textbook.workflow.js`）:

```
Workflow({
  scriptPath: "<このスキルの絶対パス>/build-textbook.workflow.js",
