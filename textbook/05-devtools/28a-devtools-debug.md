...
原典はこのコードで、`F9`（ステップ）・`F8`（再開）・Scope 編集を組み合わせた実演手順を示している。順に手を動かす。

1. 上記コードを新しい **Snippet** にコピーして実行する。実行は `debugger` の行ブレークポイントで停止する。
2. デバッガが関数宣言の横に現在値 `value = 1` をインライン表示していることに注目する。
3. Call Stack ペインで `bar()` フレームを右クリックし **Restart frame** を選ぶ。
4. **`F9`** を押して値インクリメント文をステップする。現在値が増加し `value = 2` になることに注目する。
5. 任意で、**Scope** ペインで値をダブルクリックして編集し、好きな値を設定する。
...
7. 次に Call Stack で `foo()` フレームを restart する。値が再び `0` になることに注目する。

> **Gotchas（原文）**: なぜ値が `0` に戻るのか？ — **JavaScript では引数への変更は関数の外から見えない（反映されない）。ネストされた関数は値を受け取るのであり、メモリ上の位置を受け取るのではない。**

8. **`F8`** でスクリプト実行を再開して実演を完了する。
...
原典はこの表示差を Angular デモページで確かめる手順を示している。

1. デモ https://ng-devtools.netlify.app/ を開き、**Sources** パネルで `src` > `app` > `app.component.ts` を開く。
2. `increment()` 関数にブレークポイントを設定する。
3. Call Stack セクションで **Show ignore-listed frames** チェックボックスをチェック／クリアし、「自分のコードに関連するフレームのみ」と「サードパーティを含む完全な一覧」の表示差を観察する。
...
1. ブレークポイントで実行を停止する。
2. 停止中の関数を編集する。
3. Command / Control + S を押して変更を適用する。**デバッガは関数を自動的に restart する。**
4. 実行を続ける。

原典はこの節を、これまで追ってきたデモの型バグそのもので例示している。「この例では `addend1` と `addend2` 変数が当初 誤った `string` 型を持つ。そのため数値を加算する代わりに文字列が連結される。修正のため、ライブ編集中に `parseInt()` 関数を追加する。」つまり停止したまま `addend1 + addend2` を `parseInt(addend1) + parseInt(addend2)` に書き換え、保存すれば、その場で関数が再実行されて正しい合計に直る。
...
### 5.6 Ignore List（スクリプトを無視する / 旧 blackbox）★脆弱性調査で重要

ここまで Call Stack の「Show ignore-listed frames」（§5.2.2）やファイルツリーの「Hide ignore-listed sources」（§5.3）で名前だけ出てきた **Ignore List** を、ここで正面から扱う。

#### 5.6.1 「スクリプトを無視する」とは何か

**Ignore List（無視リスト）**とは、デバッグ中にスキップしたいスクリプトを登録しておく仕組みのこと。歴史的に **blackbox（ブラックボックス）** と呼ばれていた機能で、担当範囲でいう「blackbox」はこれを指す。

原典は無視の効果をこう定義する。「**無視されたスクリプトは Call Stack ペインで隠され（obscured）、コードをステップしてもそのスクリプトの関数に決してステップインしない。**」つまり自分が書いていないコードを踏まずに、自分のコードだけを追える。
...
原典の説明はこうだ。「`A` は信頼しているサードパーティライブラリである。デバッグしている問題がそのサードパーティライブラリに関係しないと確信できるなら、そのスクリプトを無視するのが合理的である。」脆弱性調査でも、信頼できるライブラリや、アンチデバッグを仕込む拡張機能のコードを無視して、自分が追いたい経路に集中できる。

#### 5.6.2 スクリプトを無視に追加する4つの方法

無視リストへの追加口は4つある。どこからでも同じリストに入る。

| 追加する場所 | 操作 |
| --- | --- |
| ファイルツリー（Sources > Page） | ディレクトリまたはスクリプトファイルを右クリック > **Add directory/script to ignore list** |
| Editor ペイン | ファイルを開いて任意の場所を右クリック > **Add script to ignore list** |
| Call Stack ペイン | 無視したいスクリプトの関数を右クリック > **Add script to ignore list** |
| Settings | **Settings > Ignore List** で有効化・パターン登録（後述） |

無視したソースをファイルツリーで選ぶと、**警告バナー**が出る。ここで **Remove from ignored list**（無視から外す）または **Configure**（設定を開く）をクリックできる。ソースを隠している（Hide ignore-listed sources を有効にしている）場合は、**Settings > Ignore List** の一覧から個別に削除する。

#### 5.6.3 Settings > Ignore List のトグル

**Settings > Ignore List** タブに、無視の対象を決めるスイッチが並ぶ。

| 項目 | 意味 |
| --- | --- |
| **Enable Ignore Listing** | すべての ignore-listing 機能のメインスイッチ。これを切ると以下すべてが無効になる |
| **Add content scripts to ignore list** | **拡張機能のコンテンツスクリプト（content script）を無視に追加するトグル。** content script とは、Chrome 拡張が各ページに注入して動かす JavaScript のこと。全ページに `click` リスナを登録する拡張などで「自分のコードでない場所で止まる」現象を防げる |
| **Automatically add known third-party scripts to ignore list** | source maps の `ignoreList` プロパティに基づき、既知のサードパーティスクリプトを自動で無視に追加する。フレームワークやバンドラ（Angular・Nuxt など）が情報を供給する必要がある |
| **Custom exclusion rules** | 自分の正規表現（RegEx）パターンで無視対象を指定する |

> **つまずき対策**: §3.3 の「click で止まるが自分のコードでない」や、つまずきポイントで触れた `Add content scripts to ignore list` は、この **Settings > Ignore List** の拡張機能コンテンツスクリプト向けトグルのこと。シークレットモードで拡張を無効化する代わりに、このトグルで拡張の content script をまとめて無視できる。

〔攻撃・診断の観点〕Ignore List は、対象サイトが仕込む**アンチデバッグ**（`debugger` を連発する難読化スクリプトなど）や、調査に無関係なサードパーティ JS を視界から外す主要手段である。妨害スクリプトを無視に入れれば、ステップしても踏み込まなくなり、§4.5 の Force script execution と併せてデバッグ妨害を回避しやすくなる。
...
4. ダイアログに条件を入力する。条件は真偽を返す任意の JavaScript 式でよい。たとえばループ変数 `i` が特定の反復のときだけ止めたいなら `i === 6`、ある値が閾値を超えたときだけ止めたいなら `x > 10` のように書く。
5. Enter を押す。**行番号列の上に、疑問符付きのオレンジ色のアイコンが現れる。**

原典の画像キャプションは、この機能の使いどころを一言で示している。「ループ内で `x` が `10` を超えた反復 `i=6` でのみ発火した条件付き行ブレークポイント」。無関係な反復では止まらず、狙った瞬間だけ捕まえられる。
...
無効化すると、Sources パネルは行番号隣のマーカーを**半透明**にする。グループの右クリックメニューには「ファイル内の全削除（Remove all breakpoints in file）／無効化／有効化」「全ファイルの全削除（Remove all breakpoints）」「他グループの削除（Remove other breakpoints in other groups）」がある。

個別のブレークポイントは、編集中に**インラインエディタのドロップダウンで種類（type）を変更できる**。個別ブレークポイントの右クリックメニューには次がある。

- **Remove breakpoint**（このブレークポイントを削除）
