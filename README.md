...
- [GraphQL & WebSocketセキュリティを極める教科書](./graphql&websocket-textbook/docs/index.md) ― GraphQLとWebSocket/CSWSHを初級〜専門家まで（introspection・recon/Clairvoyance・graphw00f／BOLA・BFLA・nested authz bypass／★alias・array batchingでのレート制限・2FA/OTP総当りバイパスの数値化・batchQL／循環クエリ深いネストDoS・field duplication／GraphQL injection・CSRF／InQL・GraphQLmap・DVGA／WebSocketハンドシェイク／★CSWSH原理・PoC構造・RCEチェーン／メッセージ経由SQLi/XSS・Protobuf／SuiteCRM・Directus・Gitpod・Zeppelin・LogRhythm 実CVE・H1報告／OWASP・Flatt 防御）全10章＋序章＋付録
...
- [クラウド／IAM攻撃を極める教科書](./cloud-textbook/docs/index.md) ― SSRF→メタデータ(IMDS)→一時クレデンシャル窃取→IAM権限昇格→アカウント乗っ取りの連鎖を軸にAWS/GCP/Azureを攻撃者視点で（IMDSv1/v2の差とSSRFバイパス／GCP・Azureメタデータ／Capital One事件／窃取後の非破壊enumeration・CloudFox／S3バケット誤設定・権限昇格／Rhino 21手法・BishopFox 31経路・PMapper/Cloudsplaining／Lambda・サーバレス連鎖／GitHub・.env鍵漏洩Recon・TruffleHog／PACU・ScoutSuite・Prowler／HackerOne実報告・高額SSRF事例／テスト方法論と防御・flaws.cloud/CloudGoat/IAM Vulnerable）全10章＋序章＋付録
- [バグバウンティ運用・戦略・自動化・収益化を極める教科書](./strategy&manetaize-textbook/docs/index.md) ― 武器（脆弱性技術）を揃えた次の「How」を運用体系として（プログラム選定Public/Private/VDP・学術調査／継続Reconパイプライン・TomNomNom流ワードリスト／Obsidian/Notionノート術・時間管理／★nucleiカスタムテンプレート・axiom分散スキャン・差分検知→notify通知で「出力でなく発見」を生む自動化／脆弱性連鎖chaining・Meta $111,750 Path Traversal→DLL hijack→RCE事例／★レポートのタイトル・再現手順・impact物語化・triager対応・重複回避／★燃え尽きのメカニズムと持続可能な習慣／収益のべき乗分布・フルタイムの現実・コラボ/LHE・キャリア／tl;dr sec・Critical Thinking/Bug Bounty JP Podcast・書籍）全9章＋序章＋付録
